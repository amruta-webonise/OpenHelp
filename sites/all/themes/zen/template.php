...
function zen_form($variables)
{
    ...
    if ($variables['element']['#form_id'] == "user_login") {
        return '<div class="loginSignupCnt"><div class="loginBoxWrap"><div class="loginBox">' .
            '<form' . drupal_attributes($element['#attributes']) . '><div>' . $element['#children'] . '</div></form>' .
            '</div></div><div class="signupBoxWrap"><div class="signupBox"><h2>Sign up for a free account!</h2><p>Create your account to start your My Ride profile and save your favorite trips, routes and stops.</p><div class="signupCnt"><span>Manage your <br />commute quickly <br />and easily.</span>' .
            '<a href="/user/register" class="blueBtn">Sign Up &raquo;</a></div></div></div></div>';
    }
    elseif ($variables['element']['#form_id'] == "user_register_form") {
        return '<div class="createAcCnt"><form' . drupal_attributes($element['#attributes']) . '><div>' . $element['#children'] . '</div></form></div>';
    }
    elseif ($variables['element']['#form_id'] == "user_profile_form") {
...
function zen_theme(&$existing, $type, $theme, $path)
{
    ...
    $hooks['user_register_form'] = array(
        ...
        'template' => 'templates/page-user-register',
        'preprocess functions' => array('zen_preprocess_user_register_form'),
    );
...
function zen_preprocess_user_register_form(&$vars)
{
    $args = func_get_args();
    array_shift($args);
 ...
 *
 * The JS and CSS filed needs to be arranged properly. Needs to refactor
 */
function zen_preprocess_html(&$variables, $hook)
{
    // $path = drupal_get_path('theme', 'zen') . '/zen-internals/';;
    $path = '/sites/all/themes/zen/zen-internals/';
    drupal_add_js($path.'js/jquery-1.8.2.min.js', array('type' => 'external'));
    drupal_add_js($path.'js/jquery-ui-1.10.1.custom.min.js', array('type' => 'external'));
    drupal_add_css($path.'css/jquery-ui-1.10.1.custom.min.css', array('type' => 'external'));
    drupal_add_css($path . 'css/jquery.qtip.css', array('type' => 'external'));
    drupal_add_js($path . 'js/jquery.qtip.min.js', array('type' => 'external'));
    drupal_add_js($path . 'js/custom.js', array('type' => 'external'));
    if (drupal_is_front_page() == 1) {
        drupal_add_css($path . 'css/slider.css', array('type' => 'external'));
    }
    if(arg(1)=="register" || arg(0)=="user"){
     drupal_add_css($path . 'css/profile_form.css', array('type' => 'external'));
    }
    drupal_add_css($path . 'css/custom.css', array('type' => 'external'));
    if(arg(0)=="video"){
        drupal_add_js($path . 'js/jquery.colorbox-min.js', array('type' => 'external'));
        drupal_add_css($path . 'css/colorbox.css', array('type' => 'external'));

    }
    if (arg(0) == 'taxonomy') {
        drupal_add_js($path . 'js/jquery.cycle2.min.js', array('type' => 'external'));
        drupal_add_js($path . 'js/jquery.cycle2.carousel.js', array('type' => 'external'));
    }
    if (arg(0) == 'bus-route') {
        drupal_add_js('/sites/all/modules/tta/js/jquery.fancybox.pack.js', array('type' => 'external'));
        drupal_add_css('/sites/all/modules/tta/css/jquery.fancybox.css', array('type' => 'external'));
    }
    if (arg(0) == 'tripplanner') {//this is the trip planner full page where 'Plan My Trip' block is not present
        drupal_add_js('https://maps.googleapis.com/maps/api/js?sensor=false&libraries=places', array('type' => 'external'));//after removing the js from global uncomment this one
        drupal_add_js('/sites/all/modules/tta/js/jquery.fancybox.pack.js', array('type' => 'external'));
        drupal_add_css('/sites/all/modules/tta/css/jquery.fancybox.css', array('type' => 'external'));
        drupal_add_css('/sites/all/themes/zen/zen-internals/css/lightbox.css', array('type' => 'external'));
        drupal_add_js('/sites/all/modules/tripplanner/js/tripplanner.js', array('type' => 'external'));
    } else if (arg(0) == 'favorite') {
        drupal_add_js('/sites/all/modules/favorite/js/favorite.js', array('type' => 'external'));
        drupal_add_js('/sites/all/themes/zen/zen-internals/js/frontpage.js', array('type' => 'external'));
    } else {
...
function zen_preprocess_page(&$variables, $hook)
{
    ...
    if (arg(0) == 'taxonomy' && arg(1) == 'term' && is_numeric(arg(2))) {
        ...
        $tid = arg(2);
        $vid = db_query("SELECT tv.machine_name FROM taxonomy_term_data ttd inner join taxonomy_vocabulary tv on tv.vid=ttd.vid WHERE tid = :tid", array(':tid' => $tid))->fetchField();
        /*print_r($variables['theme_hook_suggestions']);die;*/
...
function zen_preprocess_user_profile(&$variables)
{
